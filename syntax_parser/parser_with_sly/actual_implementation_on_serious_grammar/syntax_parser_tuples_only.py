...
class CalcParser(Parser):
    ...
    def command_list(self, p):
        return (p.command, p.command_list)
    ...
    def variable_declaration(self, p):
        ...
        return (p.type_specifier, p.IDENTIFIER_CONST, p.SCOLON)
    ...
    def variable_declaration(self, p):
        ...
        return (p.type_specifier, p.IDENTIFIER_CONST, p.ASSIGN, p.expression, p.SCOLON)
    ...
    def function_definition(self, p):
        return (p.type_specifier, p.IDENTIFIER_CONST, p.LPAREN, p.function_parameters, p.RPAREN, p.compound_statement)

    @_('type_specifier IDENTIFIER_CONST LPAREN RPAREN compound_statement')
    def function_definition(self, p):
        return (p.type_specifier, p.IDENTIFIER_CONST, p.LPAREN, p.RPAREN, p.compound_statement)
    ...
    def parameter_list(self, p):
        return (p.parameter, p.COMMA, p.parameter_list)
    ...
    @_('stmt')
    def stmt_list(self, p):
        return { 'statement' : p.stmt }   # unconverted to tuple

    @_('if_else_variant_stmt')
    def stmt(self, p):
        return (p.if_else_variant_stmt)

    @_('non_if_else_variant_stmt')
    def stmt(self, p):
        return (p.non_if_else_variant_stmt)
    ...
    def iteration_statement(self, p):
        return (p.WHILE, p.LPAREN, p.boolean_expression, p.RPAREN, p.compound_statement)

    @_('LCURLY stmt_list RCURLY')
    def compound_statement(self, p):
        return ('block', p.LCURLY, p.stmt_list, p.RCURLY)
